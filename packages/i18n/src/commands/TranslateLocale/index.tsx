...
import { getLocaleObj } from "@/utils/getLocalObj";
...
class TranslateLocale {
  ...
  async start() {
    consola.start("Optum i18N is analyzing your project...");
  ...
  async runQuery() {
    ...
    for (const item of this.query) {
      ...
      const data = await this.i18n.translate({
        ...
        onProgress: (rest) => {
          if (rest.maxStep > 0) {
            chalk.gray(`[Progress continued: ${rest.progress}]`);
          } else {
            chalk.gray(`[Progress Complete]`);
          }
        },
      });
      chalk.gray(`[Progress Complete 2]`);
  ...
  genFolderQuery() {
    ...
    consola.info(
      `Running in ${chalk.bold.cyan("📂 Folder Mode")} and has found ${chalk.bold.cyan(files.length)} files.`
    );
